...
- Import page: failed uploads show a safe diagnostics panel with status, request ID, stage, upload path, file count, file extensions, MIME types, and file sizes. The copy button does not include filenames or statement contents.
...
- Import bug reports should use the safe diagnostics copy button plus request ID, not filenames or raw statement contents.
...
Request ID:
Import stage:
Safe diagnostics copied from app:
Expected:
Actual:
Reproduction steps:
Fix status:
