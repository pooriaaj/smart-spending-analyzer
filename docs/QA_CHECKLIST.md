...
Current focused frontend automated coverage includes API auth handling, protected-route auth gates, account selector/account-scope behavior, dashboard summary/recent-filter/manual-add behavior, login, registration, forgot/reset password, transaction form create/edit/category suggestion behavior, profile data export, password visibility, and error helpers. Manual QA is still required for full product confidence.
