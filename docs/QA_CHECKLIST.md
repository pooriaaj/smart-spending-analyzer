...
Current focused frontend automated coverage includes API auth handling, protected-route auth gates, account selector/account-scope behavior, dashboard summary/recent-filter/manual-add behavior, transaction ledger filter/edit/delete behavior, analytics summary/filter/drilldown behavior, import account-gate/table-review/confirm behavior, assistant status/history/scoped-question behavior, budget summary/suggestion/save/delete behavior, login, registration, forgot/reset password, transaction form create/edit/category suggestion behavior, profile export/update/password/learning/delete-guard behavior, password visibility, and error helpers. Manual QA is still required for full product confidence.
