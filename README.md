...
- Current focused frontend coverage includes API auth handling, protected route gates, account selector/account-scope behavior, dashboard summary/recent-filter/manual-add behavior, transaction ledger filter/edit/delete behavior, analytics summary/filter/drilldown behavior, import account-gate/table-review/confirm behavior, assistant status/history/scoped-question behavior, login, registration, forgot/reset password, transaction form create/edit/category suggestion behavior, profile export, password visibility, and error helpers.
- GitHub workflows:
